...
def make_ohlcv(n: int, close_prices=None, volume=None) -> pd.DataFrame:
    ...
    closes = close_prices if close_prices is not None else [100.0 + i * 0.1 for i in range(n)]
...
def make_rule(long: bool, short: bool, strength: float = 0.5) -> dict:
    return {
        "rule": "Test Rule", "description": "test",
        "long": long, "short": short, "strength": strength,
        "value": "test", "signal_hint": "test", "error": False,
    }
...
def patch_analyze(long_flags, short_flags, funding=None, price_df=None):
    ...
    rule_fns = [
        "check_rule_1_trend", "check_rule_2_rsi", "check_rule_3_macd",
        "check_rule_4_ema_stack", "check_rule_5_volume", "check_rule_6_stoch_rsi",
    ]
    patches = [
...
class TestRule3MacdHistogram:
    ...
    def test_returns_all_required_fields(self):
        with patch("analyzer.fetch_ohlcv", return_value=None):
            result = analyzer.check_rule_3_macd("BTC/USDT")
        for f in ("rule", "description", "long", "short", "strength", "value", "signal_hint", "error"):
            assert f in result
...
class TestRule5OBVTrend:
    ...
    def test_rule_name_reflects_obv_logic(self):
        ...
        with patch("analyzer.fetch_ohlcv", return_value=make_ohlcv(60, close_prices=closes)):
    ...
    def test_long_when_obv_trending_up(self):
        ...
        closes = [100.0 + i * 0.5 for i in range(60)]
        with patch("analyzer.fetch_ohlcv", return_value=make_ohlcv(60, close_prices=closes, volume=[500.0]*60)):
            result = analyzer.check_rule_5_volume("BTC/USDT")
        assert result["long"] is True
        assert result["short"] is False
        assert result["error"] is False
    ...
    def test_strength_in_range_when_long_passes(self):
        ...
        with patch("analyzer.fetch_ohlcv", return_value=make_ohlcv(60, close_prices=closes, volume=[1000.0]*60)):
            result = analyzer.check_rule_5_volume("BTC/USDT")
        if result["long"]:
            assert 0.0 < result["strength"] <= 1.0
...
class TestFetchFundingRate:
    ...
    def test_neutral_rate_not_blocked(self):
        with patch.object(analyzer.exchange, "fetch_funding_rate", return_value={"fundingRate": 0.0001}):
    ...
    def test_extreme_positive_blocks_long(self):
        with patch.object(analyzer.exchange, "fetch_funding_rate", return_value={"fundingRate": 0.0006}):
            result = analyzer.fetch_funding_rate("BTC/USDT")
    ...
    def test_exactly_at_positive_threshold_is_extreme(self):
        ...
        with patch.object(analyzer.exchange, "fetch_funding_rate", return_value={"fundingRate": 0.0005}):
            result = analyzer.fetch_funding_rate("BTC/USDT")
        assert result["extreme"] is True
        assert result["blocked_side"] == "LONG"
    ...
    def test_just_below_positive_threshold_is_not_extreme(self):
        with patch.object(analyzer.exchange, "fetch_funding_rate", return_value={"fundingRate": 0.0004}):
            result = analyzer.fetch_funding_rate("BTC/USDT")
        assert result["extreme"] is False
        assert result["blocked_side"] is None

    def test_exchange_error_returns_safe_defaults(self):
        with patch.object(analyzer.exchange, "fetch_funding_rate", side_effect=Exception("timeout")):
            result = analyzer.fetch_funding_rate("BTC/USDT")
...
class TestAnalyzeSixRules:
    ...
    def test_funding_block_does_not_affect_already_wait(self):
        ...
        result = self.run([True, True, True, False, False, False], [False]*6, funding=BLOCK_LONG)
        assert result["signal"] == "WAIT"
